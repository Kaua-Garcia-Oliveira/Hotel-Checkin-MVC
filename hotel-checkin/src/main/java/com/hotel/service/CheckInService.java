...
public class CheckInService {
    ...
    public void realizarCheckout(int checkInId) throws Exception {
        ...
        if (checkIn == null) throw new Exception("Check-in não encontrado.");
        if (!"ATIVO".equals(checkIn.getStatus())) throw new Exception("Este check-in já foi finalizado.");
        checkInDAO.realizarCheckout(checkInId);
        quartoDAO.atualizarDisponibilidade(checkIn.getReserva().getQuarto().getId(), true);
